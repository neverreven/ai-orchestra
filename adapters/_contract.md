...
> Every adapter under `{{INSTALLED_FOLDER}}/adapters/<ide>/` must satisfy this contract end-to-end. The contract is the binding between the orchestra **core** (project-agnostic, tool-agnostic markdown specs) and the **target project's IDE-native files**.
...
|-----------|---------|-----------|
| Skill referenced in `AGENTS.md` | `{{INSTALLED_FOLDER}}/core/skills/audit/cleanup/SKILL.md` | `.cursor/skills/cleanup/SKILL.md` |
| Skill referenced in `CLAUDE.md` | `{{INSTALLED_FOLDER}}/core/skills/audit/pre-release/SKILL.md` | `.cursor/skills/pre-release/SKILL.md` |
| IDE-native skill (rendered copy) | May use IDE path; must note it is a rendered copy | — |

**Rationale:** `AGENTS.md` is read by all agents in all IDEs. A reference to `.cursor/skills/` embedded in it silently breaks every non-Cursor session. The canonical skill spec lives in `{{INSTALLED_FOLDER}}/core/skills/` and is always present regardless of IDE. Adapters may also install a rendered IDE copy alongside it, but the cross-IDE reference must always point to the core.

**In practice:** when an adapter generates an `AGENTS.md` section that lists available skills (e.g., in a "Skills" table or bullet list), each entry must link to `{{INSTALLED_FOLDER}}/core/skills/<cat>/<slug>/SKILL.md`. The IDE-native rendered copy path is supplementary, not the primary reference.
...
| Field | Source | Notes |
|-------|--------|-------|
| `orchestra.version` | `score/VERSION` (or `ai-orchestra/VERSION` for legacy installs — see `installedFolder` in `.ai-orchestra/install.json`) | The core version. |
...
Adapters are **read-only against the orchestra core**. They never write to `{{INSTALLED_FOLDER}}/core/`. They never write to `{{INSTALLED_FOLDER}}/adapters/` (their own folder is read-only at install time too). Only the target project and the user's `~/.ai-orchestra/` are written to.
