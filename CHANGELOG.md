...
## [3.2.2] - Fix: CLI init output

### Fixed

- **`bin/init.mjs`** -- `logNextSteps()` printed `Then ask your agent: 'upgrade orchestra'` five times in the published v3.2.1 package due to a stale snapshot being packaged during the AGPLv3 release commit. The source code was already correct; this patch republishes the corrected file.
