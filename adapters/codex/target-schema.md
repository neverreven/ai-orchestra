...
├── ai-orchestra/                             # orchestra core (referenced, not modified)
│   └── core/skills/<category>/<skill-id>/SKILL.md   # skills executed in-place
└── AGENTS.md                                  # consolidated context (§2)
```

The Codex install touches strictly fewer files than Cursor or Claude Code because skills are not duplicated. The orchestra core (`ai-orchestra/`) is required to remain in the project; its files are READ by the agent during skill execution, not COPIED.
...
- **Source:** `ai-orchestra/core/skills/<category>/<skill-id>/SKILL.md` (the agent navigates to this path on trigger match — substitute the actual `<category>` and `<skill-id>` for each installed skill)
...
- Skills: `ai-orchestra/core/skills/<category>/<skill-id>/SKILL.md` (executed in place — never duplicated)
- Learnings: `<learnings-path>` (recorded in install marker)
- Install marker: `.ai-orchestra/install.json`
- Orchestra core: `ai-orchestra/` (must remain in the repo for skills to work)
...
```json
{
  "mcpServers": {
    "orchestra-analytics-database": {
      "command": "echo",
      "args": [
        "Slot 'orchestra-analytics-database' was registered by ai-orchestra. Replace this entry with your real MCP server: command + args + env. See ai-orchestra/adapters/codex/mcp.md."
      ],
      "metadata": {
        "orchestra": true,
