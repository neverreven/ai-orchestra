...
> The full mapping table: every orchestra core artifact, the target path in a Cursor project, the action used when applying it, and the conflict-handling rule when a target already exists. Every entry references the source file in `{{INSTALLED_FOLDER}}/core/`.
