...
- Schema-conformant files are easier for an agent to read at install time ó every adapter knows where to find each section without parsing prose.
...
- All links in `## References` resolve to existing files inside the orchestra spec folder (`score/` for v3+ installs, or `ai-orchestra/` for legacy).
...
- No file links to a path outside the orchestra spec folder.

For **URL hygiene** (applies to every markdown file under the orchestra spec folder (`score/` or legacy `ai-orchestra/`), not just roles and skills):
...
- Verification command: `grep -nrE "https?://[^/]*ai-orchestra" score/` (or `grep -nrE "https?://[^/]*ai-orchestra" ai-orchestra/` for legacy installs) must return zero hits **outside** `_test-fixtures/` and `core/_lint.md`.
...
- Otherwise the lint scope for this rule is `score/core/`, `score/adapters/` (or `ai-orchestra/core/`, `ai-orchestra/adapters/` for legacy), and the top-level documentation files (`README.md`, `RUN.md`, `CHANGELOG.md`, `MIGRATION.md`, `_v1.x-backlog.md`, `_v3.x-backlog.md`).
...
- `findings: array` of:
  ...
  - `target_path: string` (path inside the orchestra spec folder)
...
| `cross.external-link` | critical | A link points outside the orchestra spec folder. |
...
- The default scope is the orchestra's own content (`score/core/roles/`, `score/core/skills/`) ù or `ai-orchestra/` equivalents for legacy. The linter does not validate target-project files.
...
- Style rules (sentence length, passive voice) ó explicitly out of scope for v1; the linter is structural.
...
- [`roles/_schema.md`](roles/_schema.md) ó role file schema.
- [`skills/_schema.md`](skills/_schema.md) ó skill file schema.
- [`skills/audit/ai-infra-audit/SKILL.md`](skills/audit/ai-infra-audit/SKILL.md) ó runner skill (spec ships in PR 3).
- [`registry/install.schema.md`](registry/install.schema.md) ó install marker that the audit reads alongside the lint result.
