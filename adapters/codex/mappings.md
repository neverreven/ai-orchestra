...
- Path to the source `SKILL.md` (e.g., `{{INSTALLED_FOLDER}}/core/skills/audit/ai-infra-audit/SKILL.md`).
...
- The skill's `source` path resolves to the orchestra core (`{{INSTALLED_FOLDER}}/core/skills/<category>/<skill-id>/SKILL.md`).
...
- The orchestra core MUST remain in the repo. Deleting `{{INSTALLED_FOLDER}}/` breaks skill execution. The audit warns when the core is missing.
...
2. Saying "review this session for learnings" → instructs the agent to (a) evaluate the scheduler via `{{INSTALLED_FOLDER}}/core/scheduler/RUNNER.md §0–§3`, then (b) walk the conversation and propose entries for the learnings doc.
...
- [`render-rules.md`](render-rules.md) — exact rendering details for managed sections, skill catalog, and the idempotency contract.
