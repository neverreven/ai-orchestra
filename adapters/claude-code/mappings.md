...
The body is the source `SKILL.md` content from the first `# <Skill Name>` heading onward, with relative links rewritten from orchestra-relative to project-relative (the source file lives at `{{INSTALLED_FOLDER}}/core/skills/<category>/<skill-id>/SKILL.md`; the rendered file lives at `.claude/commands/<skill-id>.md`).
