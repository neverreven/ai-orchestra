...
// Usage:
//   npx @neverreven/ai-orchestra init [--force] [--include-fixtures] [--target=<path>] [target-dir]
...
//   npx @neverreven/ai-orchestra chat [--port=<PORT>]
...
COMMANDS
  ...
  chat              Open the local ensemble web chat UI in the default browser.
                    Requires the ensemble to be running with ENABLE_WEB_UI=true.

INIT OPTIONS
  [target]            Project root to install into (defaults to current dir).
  --target=<path>     Explicit target path (alternative to positional argument).
...
CHAT OPTIONS
  --port=<PORT>       Port the ensemble web chat is running on (default: 3847).
                      Auto-detected from ~/.ai-orchestra/ensemble/.env if omitted.
...
WHAT chat DOES
  Reads the ensemble port from ~/.ai-orchestra/ensemble/.env (or --port flag),
  constructs http://localhost:PORT, and opens it in the default browser.
  The web chat UI streams responses from the Lead agent in real time.
...
async function updateInstallMarkerTier(markerDir, tier, ensembleUpdates = {}) {
  ...
  try {
    ...
    // Apply any additional ensemble field updates
    Object.assign(marker.ensemble, ensembleUpdates);
...
async function cmdInit() {
  ...
  // Support both --target=<path> flag and positional[1]
  const targetFlag = args.find((a) => a.startsWith('--target='));
  const targetRoot = path.resolve(
    targetFlag ? targetFlag.slice('--target='.length) : (positional[1] ?? '.')
  );
...
async function cmdSetupEnsemble() {
  ...
  // Resolve install location: --location=system-global or default project-local
  const locationFlag = args.find((a) => a.startsWith('--location='));
  const location = locationFlag ? locationFlag.slice('--location='.length) : 'project-local';
  const isSystemGlobal = location === 'system-global';

  const targetRoot = path.resolve(positional[1] ?? '.');
  const pkgRoot = fileURLToPath(new URL('..', import.meta.url));
  const ensembleSrc = path.join(pkgRoot, 'ensemble');

  const ensembleDst = isSystemGlobal
    ? path.join(os.homedir(), '.ai-orchestra', 'ensemble')
    : path.join(targetRoot, '.ai-orchestra', 'ensemble');

  const markerDir = isSystemGlobal
    ? path.join(os.homedir(), '.ai-orchestra')
    : path.join(targetRoot, '.ai-orchestra');

  if (!isSystemGlobal && !existsSync(targetRoot)) {
    console.error(`  ERROR: target directory does not exist: ${targetRoot}`);
    process.exit(1);
  }

  // For project-local, require Tier 1 to already be present
  if (!isSystemGlobal) {
    ...
    if (!scoreExists) {
      ...
      console.error('         Or re-run with --location=system-global to install at system level.');
      process.exit(1);
    }
  }

  if (isSystemGlobal) {
    console.log(`  Install location: system-global (${ensembleDst})`);
    console.log('  This ensemble will be shared across all projects on this machine.');
    console.log('');
  }
  ...
  console.log('');
  if (isSystemGlobal) {
    console.log('  To bootstrap score/ into another project from the Lead agent:');
    console.log('    Ask the Lead agent: "add project at /path/to/your-project"');
    console.log('');
  }
  console.log('  To open the web chat (if enabled):');
  console.log('    npx @neverreven/ai-orchestra chat');
  console.log('');
...
function readEnvPort(envFilePath) {
  if (!existsSync(envFilePath)) return null;
  try {
    const lines = readFileSync(envFilePath, 'utf8').split('\n');
    for (const line of lines) {
      const trimmed = line.trim();
      if (trimmed.startsWith('WEB_UI_PORT=')) {
        const val = trimmed.slice('WEB_UI_PORT='.length).trim();
...
async function openInBrowser(url) {
  ...
  return new Promise((resolve) => {
    const child = spawn(openCmd, [url], { stdio: 'ignore', shell: platform === 'win32', detached: true });
    child.unref();
    child.on('error', () => {}); // Ignore errors — user can always open manually
    setTimeout(resolve, 500); // Small delay so the OS can accept the command
  });
}
...
async function cmdChat() {
  ...
  // Resolve port: --port=N > ~/.ai-orchestra/ensemble/.env > project-local .env > default
  const portFlag = args.find((a) => a.startsWith('--port='));
  let port = portFlag ? parseInt(portFlag.slice('--port='.length), 10) : null;

  if (!port) {
    const globalEnv = path.join(os.homedir(), '.ai-orchestra', 'ensemble', '.env');
    const localEnv = path.join('.', '.ai-orchestra', 'ensemble', '.env');
    port = readEnvPort(globalEnv) ?? readEnvPort(localEnv) ?? 3847;
  }

  const url = `http://localhost:${port}`;

  console.log(`  Opening ensemble web chat at ${url}`);
  console.log('');
  console.log('  If the browser does not open, navigate to the URL above manually.');
  console.log('  The ensemble must be running (bun run dev:lead) for the chat to connect.');
  console.log('');
