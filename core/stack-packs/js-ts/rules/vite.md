...
- **`build.rollupOptions.output.manualChunks` for known-large vendor splits.** When the app reliably uses heavy libraries (Excalidraw, ReactFlow, CKEditor), explicitly chunk them so the main bundle stays small. Audit the bundle with `rollup-plugin-visualizer`.
