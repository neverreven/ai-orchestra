...
const bus = openBus({ stateRoot: STATE_ROOT, actor: "devops" });
...
setInterval(() => {
  ...
  logger.debug("heartbeat", { role: "devops", pid: process.pid });
}, 5_000);

logger.info(`Starting devops agent (cwd: ${config.cwd})`);
