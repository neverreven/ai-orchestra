...
VS Code custom prompts are single files. Auxiliary files from the source skill folder are NOT copied in v1. Skills that need auxiliary content reference paths inside `{{INSTALLED_FOLDER}}/core/skills/<category>/<skill-id>/` from within the prompt body. This is a declared gap; see [`INSTALL.md`](INSTALL.md) §6.
