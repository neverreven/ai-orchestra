...
| `core.preserved` | `score/` directory (or legacy `ai-orchestra/`) remains in the project root. | Directory check (`installedFolder` field in `.ai-orchestra/install.json` determines which name to check). | Exists. | Absent. | critical |
