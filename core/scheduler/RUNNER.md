...
|-------------|------------|
| `skill` | Read the skill file at `score/core/skills/<category>/<skill_id>/SKILL.md` (or `ai-orchestra/core/skills/...` for legacy installs — check `installedFolder` in `.ai-orchestra/install.json`). Follow the skill's `## Process` section exactly. Pass `mode: "scheduled"` as context so the skill knows it was triggered automatically rather than by the user. |
