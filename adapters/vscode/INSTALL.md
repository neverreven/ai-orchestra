...
| Skill folder mirroring (auxiliary `template.md`, `examples/`) | Not used — Copilot custom prompts are single files. | `.github/prompts/<id>.prompt.md` has no folder convention. | Skills that need auxiliary content reference paths inside `{{INSTALLED_FOLDER}}/core/skills/<category>/<skill-id>/` from inside the prompt body. |
